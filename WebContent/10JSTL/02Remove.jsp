...
<body>
	<!-- 
		remove태그 : set으로 선언한 변수를 제거할때 사용하는 태그
			영역에 저장된 속성을 제거하므로 로그아웃에서 사용할 수 도있다
	 -->
	<h2>remove  태그</h2>
	...
	<!-- 
	xxxscope 내장객체를 사용하지 않으면 가장 좁은 영역의 변수를 읽어온다.
	 -->
	<h3>삭제전 출력하기</h3>
	<ul>
		<li>페이지 : ${pageVar }</li>
	...
	영역 혹은 변수명이 설정이 달못되더라도 에러는 발생하지 않는다.
	 -->
	 <h3>remove로 삭제하기</h3>
	...
	<!-- 
		영역에 동일한 이름의 속성이 존재하는 경우에는 모든영역의 
	...
	<ul>
		<li>페이지 : ${pageVar }</li> <!-- 삭제 -->
		<li>리퀘스트1 : ${requestScope.pageVar }</li> <!-- 삭제 -->
		<li>리퀘스트2 : ${requestVar }</li>
		<li>세션 : ${sessionVar }</li>
		<li>어플리케이션 : ${appVar }</li>
	</ul>
