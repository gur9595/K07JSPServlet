...
<body>
	...
	set태그 속성ㅇ
		...
		scope : 영역을 지정 문자열만 가능하고 4가지 영역을 지정할수있음
				기본값은 가장좁은 page영역.
	즉, set태그를 사용하여 변수를 저장하면 지정된영역에 저장된다.
	 -->
	<h2>set 태그 </h2>
	<h3>set 태그로 EL에서 사용할 변수 설정하기</h3>
	<ul>
		<li>
		1.value속성에 직접 값 대입:
	...
	<!-- EL로출력시에는 별도의 형변환이 필요없고 속성명이 중복되지 않는다면 
	XXXScope내장객체를 생략할수도있다 -->
	<h3>EL로 출력</h3>
	<ul>
	...
	<!-- 
	set태그의 target, property속성은 자바빈 객체의 속성혹은
	...
	scope 속성은 var 속성이 지정된 태그에서만 지정가능함
	targer과 property를 사용해서 자바빈이나 컬렉션에 값을
	...
	<!-- 
	List계열의 컬랙션은 배열처럼 순서를 보장하고 인덱스로 접근되므로
	객체에 접근시에는 인덱스를 사용한다.
	 -->
	<h2>List컬렉션 이용하기</h2>
	<%
	...
	<h4>2단계: target 및 property속성으로 속성값변경</h4>
	<!-- BTS 정보가 출력됨 -->
