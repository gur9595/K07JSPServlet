...
<body>
	...
	<h3>자바코드로 영역및 파라미터 읽기</h3>
	<h4>영역에 저장된값</h4>
	<%
	/*
	Java코드를 통해 영역에 접근해서 속성을 가져올깨는 아래와 같이한다
	1. getAttribute()로 속성을 가져온다
	2. 형변환한다
	3. gettet()로 저장된 값을 출력한다.
	*/
	...
	<ul>
		...
		<!-- String 과 같은 자바의 기본객체는 형변환없이 출력가능 -->
		<li>
	...
	<%
	/*
	파라미터로 전달되는 값은 항상  String 이므로 산술연산을
	위해서는 숫자로 변환이 필요하다.
	*/
	int fNum= Integer.parseInt(request.getParameter("firstNum"));
	int sNum= Integer.parseInt(request.getParameter("secondNum"));
	%>
	두파라미터의 합 : <%=fNum+sNum%> <!-- 연산의 결과 500 출력됨 -->
	
	<!-- 
		속성명이 유일하다면 xxxScope내장객체 없이 읽어올수 있고
		형변환필요없고 숫자로 변경하지 않아도 산술연산이 가능함
		getter() 호출없이 맴버변수의 이름으로 저장된 값을 읽을수 있음.
	 -->
	<h3>EL로 영역과 파라미터로 전달된 값읽기</h3>
	...
	<ul>
		...
		<li>
		String 객체 : ${requestScope.strObj }
		</li>
		<li>
		Integer 객체 : ${integerObj }
		</li>
	</ul>
	
	<%--
		EL의 param내장객체를 통해 파라미터를 읽을 떄
		1. param.폼이름
		2. param['폼이름']
		3. param["폼이름"]
		모두 가능한 표현이다
