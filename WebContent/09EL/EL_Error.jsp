...
<fieldset>
	...
	<!-- 
		EL은 null에대해 어느정도까지는 처리 해주므로 아래는 
		에러가 발생하지는 않는다
	 -->
	...
	<h2>인덱스 접근시 배열이나 리스트계열 컬렉션</h2>
	<%
	//문자열배열 선언후  List컬렉션에 각요소값을 추가
	...
	<!-- 배열이나 컬렉션처럼 인덱스로 접근하는 경우 (.)닷은 사용할수없다 -->
	<ul>
	...
	<!-- JSTL의 forEach 캐그를 통해 반복출력 해당 문장은
	배열이나 컬렉션의 크기만큼 반복하는 화장 for문의 형태로 기술됨 -->
	<h4>배열</h4>
	...
	<ul>
		<li>
		존재하지 않는속성 : \${eldto.addr }
		:
		\${eldto.addr } [PropertyNotFoundException 발생]
		</li>
		<!-- DTO객체에서 getId()를 주석처리후 테스트 해볼것 -->
		<li>
		속성은 존재 , getter가 없는 경우 : ${eldto.id }
		:
		\${eldto.id } [500에러발생]
		</li>
		<li>
		el변수명이 틀린경우 : \${noeldto.pwd }
		:
	...
	<ul>
		<li>\${requestScope.nop } : ${requestScope.nop }
			[Scope 계열 내장객체, 에러 No, 미출력 -> 속성명은 개발자 마음대로 정할수 있기때문]
		</li>
		<!-- 
		...
		<li>\${pageContext.nop } : \${pageContext.nop }
			[컴파일 오류발생 -> 내장 객체명은 정해져있기떄문]
		</li>
	</ul>
	
	<h2>EL의 내장객체중 사용자가 정의한 속성에 접근하는 내장객체및 맵 컬렉션인 경우</h2>
	<%
	request.setAttribute("reqProperty", "리퀘스트 영역저장");
	Map map =new HashMap();
	map.put("mapKey", "멥에 저장한 문자열");
	%>
	<c:set var="elMap" value="<%=map %>"/>
	<ul>
		<li>\${requsetScope.reqProperty } : ${requestScope.reqProperty } [정상]</li>
		<li>\${requsetScope.REQProperty } : ${requestScope.REQProperty } [에러 NO 미출력]</li>
		<li>\${elMap.mapKey } : ${elMap.mapKey } [정상]</li>
		<li>\${elMap.MAPKey } : ${elMap.MAPKey } [에러 NO 미출력]</li>
	</ul>
