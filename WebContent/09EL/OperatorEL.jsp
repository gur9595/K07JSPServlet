...
<body>
	...
	하지만 EL 에서는 null을 0으로 간주하여 계산한다
	따라서 NullpointException 이 발생하지 않는다
	*/	
	//int a =null +10 ; <- null과의 연산이므로 에러발생
	%> 
	...
	<%
	/* 
	EL에서는 JSP에서 선언한 변수는 직접 사용할수 없다 값은 출력되지 않고
	null로 인식하게된다.
	JSP의 변수를 EL에서 사용할수 없는 이유는 EL은 4가지영역에 저장된
	속성들만 사용하지 때문이다 JSTL도 동일한 특성을 가지고있다.
	*/
	 String varScriptLet="스크립트렛 안에서 변수 언언";
	%>
	<!-- 
	...
	<!-- 
	JSP코드에서 선언한 변수를 EL에서 사용해야할 경우에는 JSTL의 
	...
	<!-- 
		...
		하지만 개발시에는 실제 서비스할 웹서버의 버전을 확인후
		사용여부를 결정한다
	...
	단 실제 숫자 비교시에는 일반적인 숫자 비교가 이루어진다.
	 -->
	<h3>EL의 비교연산자</h3>
	...
	<ul>
		...
		<!-- 
		Java에서는 문자열을 비교할때 equalsㅁ[소드르 ㄹ통해비교하지만
		EL에서는 ==형태로 비교한다
		 -->
		<li>\${"JAVA"=='JAVA' } : ${"JAVA"=='JAVA' }</li> <!-- 참&&거짓 => 거짓 -->
		<li>\${"Java"=='JAVA' } : ${"Java"=='JAVA' }</li> <!-- 거짓||참 => 참 -->
	</ul>
	...
	null이거나 "(빈문자열)일때
		...
		true를 반환하는 연산자이다
	 -->
	<h3>EL의 empty 연산자 :null일때 true를 반환하는연산자</h3>
	<%
		String nullStr = null;
	String emptyStr= "";
	...
	<ul>
		...
		<li>\${not empty emptyStr } : ${not empty emptyStr }</li>
		<li>${empty ellengthZero ? "배열의 크기가0임" : "배열의크기가 0아님" }</li>
