...
/* 
//방법1 : count(*) 함수를 통해 회원이 존재유무만 판단한다.
//		따라서 회원의 아이디, 패스워드 이외의 정보는 DB에서 가져올수없다.
boolean isMember = dao.isMember(drv, url);
...
/* MemberDTO memberDTO = dao.getMember(id, pw);
...
if(memberDTO.getId()!=null){
	...
	//저장된 값이 없다면 .. 리퀘스트 영역에 오류메세지를 저장허고 포워드한다.
	request.setAttribute("ERROR_MSG", "넌 회원이 아니군 ㅡㅡ;");
	request.getRequestDispatcher("Login.jsp").forward(request, response);
} */

//방법3 : Map컬랙션에 회원정보 저장후 반환받기
