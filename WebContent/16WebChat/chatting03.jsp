...
<body>
	<!-- 
		...
		차후 회원제 어플리케이션에서는 화원아이디 혹은 대화명으로 대체한다
	 -->
	...
	<script>
		...
		//접속자ID를 가져오는 부분(현제는 랜덤하게 생성되는 세션아이디)
		...
		*/
		...
		//웹소켓 서버가 메세지를 받은후 클라이언트에게 Echdo할떄
		function wsMessage(event){
			//메세지를 | 구분자로 spilt(분리)한다
			var message= event.data.split("|");
			//메세지의 첫번째부분은 전송한 사람의 아이디
			var sender = message[0];
			//두번째 부분은 메세지
			var content = message[1];
			if(content == ""){
				//전송한 메세지가 없다면 아무일도 하지않는다.
			}else{
				//내용이 있는 경우에만 메세지찬에 내용을 추가한다
		...
		function sendMessage(){
			...
			/*
			대화내용에 접속자아이디(세션아이디)를 파이프기호(|)로 연결한후
