...
public class ListCtrl extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(searchWord!=null) {
			...
			addQueryString = 
					String.format("seachColumn=%s&searchWord=%s&", searchColumn,searchWord);
		...
		//전체 카운트수를 Map저장함 차후 View로 전달함
		param.put("totalCount", totalRecordCount);
		
		//테이블의 전체 ResultSet을 가져옴 (페이지처리x) 
		List<DataroomDTO> lists= dao.selectList(param);
		
		//커넥션풀에 커넥션객체 반납
		dao.close();
		
		//컬렉션에 저장돤 데이터를 View로 전달하기위해 Request영역에 속성저장
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		/*
		 ...
		 처리는 doGet()메소드에서 처리할수있도록 모든 요청을 토스한다 
		 */
		doGet(req, resp);
	}
}
